...
class AnalyticsService {
  ...
  Future<Map<String, dynamic>> getTopPurposeFromDbRaw({
    ...
    try {
      ...
      var query =
          client.from(SupabaseConfig.queueEntriesTable).select('purpose');
      if (department != null) {
        query = query.eq(SupabaseConfig.departmentColumn, department);
      }
      final rows = await query;
  ...
  Future<Map<String, dynamic>> getComprehensiveAnalytics({
    ...
    try {
      ...
      print(
          'Filters: department=$department, course=$course, purpose=$purpose, month=$month, day=$day');
      ...
      final effectiveDept =
          (department == 'ALL' || department == null) ? null : department;
      ...
      if (effectiveDept != null ||
        ...
        if (day != null) {
          ...
          query = query
              .gte(SupabaseConfig.timestampColumn,
        ...
        print(
            'Fetched ${filteredEntries.length} entries from Supabase with filters');
        if (filteredEntries.isNotEmpty) {
          print(
              'Sample parsed entry - Department: ${filteredEntries.first.department}, Purpose: ${filteredEntries.first.purpose}, Course: ${filteredEntries.first.course}');
        }
      } else {
        ...
        print(
            'Fetched ${filteredEntries.length} entries from Supabase (all data)');
        if (filteredEntries.isNotEmpty) {
          print(
              'Sample entry - Department: ${filteredEntries.first.department}, Purpose: ${filteredEntries.first.purpose}, Course: ${filteredEntries.first.course}');
          ...
          for (final entry in filteredEntries) {
            deptCounts[entry.department] =
                (deptCounts[entry.department] ?? 0) + 1;
            purposeCounts[entry.purpose] =
                (purposeCounts[entry.purpose] ?? 0) + 1;
            if (entry.course != null && entry.course!.isNotEmpty) {
              courseCounts[entry.course!] =
                  (courseCounts[entry.course!] ?? 0) + 1;
            }
          }
      ...
      final waitingCount =
          filteredEntries.where((e) => e.status == 'waiting').length;
      final servingCount =
          filteredEntries.where((e) => e.status == 'current').length;
      ...
      // Get same-day demographic analytics (age and gender)
      final dailyDemographics = _getDailyDemographics(
        filteredEntries,
        selectedDay: day,
      );
      ...
      final graphData = {
        ...
        'daily_demographics': dailyDemographics,
      };
      ...
      print(
          'Total entries: $totalEntries, Waiting: $waitingCount, Serving: $servingCount, Completed: $completedCount');
      ...
      if (graphData.containsKey('department_usage')) {
        print(
            'department_usage in graphData: ${graphData['department_usage']}');
      } else {
        print('ERROR: department_usage NOT in graphData!');
      }
      if (graphData.containsKey('top_courses_usage')) {
        print(
            'top_courses_usage in graphData: ${graphData['top_courses_usage']}');
      } else {
      ...
      if (purposeStats.isNotEmpty) {
        print(
            'Sample purpose data (first 3): ${purposeStats.take(3).toList()}');
      } else {
        print('WARNING: Purpose stats is empty!');
      }

      print(
          'Analytics service: Successfully returning result with ${graphData.keys.length} graph keys');
      final graphs = result['graphs'] as Map<String, dynamic>?;
      print(
          'Analytics service: department_usage in result: ${graphs?.containsKey('department_usage') ?? false}');
      print(
          'Analytics service: top_courses_usage in result: ${graphs?.containsKey('top_courses_usage') ?? false}');
      return result;
    } catch (e, stackTrace) {
      ...
      try {
        ...
        return {
          ...
          'summary': {
            ...
            'serving': allEntries
                .where((e) => e.status == 'current' || e.status == 'serving')
                .length,
            'completed': allEntries
                .where((e) => e.status == 'done' || e.status == 'completed')
                .length,
            'incomplete': allEntries
                .where((e) => e.status == 'missed' || e.status == 'incomplete')
                .length,
            'avg_wait_time': 0.0,
          },
          'graphs': {
            ...
            'daily_demographics': _getDailyDemographics(
              allEntries,
              selectedDay: day,
            ),
          },
    ...
    try {
      ...
      for (final entry in completedEntries) {
        if (entry.countdownStart != null) {
          final waitTime =
              entry.countdownStart!.difference(entry.timestamp).inMinutes;
          if (waitTime > 0) {
  ...
  List<Map<String, dynamic>> _getPurposeDistribution(List<QueueEntry> entries) {
    ...
    final result = purposeMap.entries.map((entry) {
      final percentage =
          total > 0 ? (entry.value / total * 100).roundToDouble() : 0.0;
  ...
  Map<String, dynamic> _calculateEfficiencyMetrics(List<QueueEntry> entries) {
    ...
    final throughput =
        total > 0 ? (total / 24.0).roundToDouble() : 0.0; // per hour
  ...
  List<String> _generateInsights(List<QueueEntry> entries, String? department) {
    ...
    if (department != null) {
      final deptEntries =
          entries.where((e) => e.department == department).toList();
    ...
    final List<Map<String, dynamic>> hourlyStats =
        _getHourlyDistribution(entries);
    if (hourlyStats.isNotEmpty) {
  ...
  List<Map<String, dynamic>> _getTopCoursesUsage(List<QueueEntry> entries) {
    ...
    print(
        '_getTopCoursesUsage: Found $entriesWithCourse entries with courses, ${courseMap.length} unique courses');
  ...
  Map<String, dynamic> _getDailyDemographics(
    List<QueueEntry> entries, {
    ...
    DateTime focusDay;
    List<QueueEntry> focusEntries;

    if (selectedDay != null) {
      focusDay = DateTime(selectedDay.year, selectedDay.month, selectedDay.day);
      focusEntries = _getEntriesForDay(entries, focusDay);
    } else {
      focusDay = today;
      focusEntries = _getEntriesForDay(entries, today);

      if (focusEntries.isEmpty && entries.isNotEmpty) {
        final latestEntry = entries.reduce(
          (current, next) =>
              current.timestamp.isAfter(next.timestamp) ? current : next,
        );
        focusDay = DateTime(
    ...
    final ageBucketCounts = <String, int>{
      '17 & below': 0,
      '18-20': 0,
      '21-25': 0,
      '26-30': 0,
    ...
    for (final entry in focusEntries) {
      final genderLabel = _normalizeGenderLabel(entry.gender);
      genderCounts[genderLabel] = (genderCounts[genderLabel] ?? 0) + 1;

      final age = entry.age;
      if (age != null && age > 0) {
        validAges.add(age);
        final ageBucket = _getAgeBucketLabel(age);
        ageBucketCounts[ageBucket] = (ageBucketCounts[ageBucket] ?? 0) + 1;
      }
    }

    final genderData = genderCounts.entries
        .map(
          (entry) => {
            'label': entry.key,
            'count': entry.value,
            'percentage': focusEntries.isNotEmpty
                ? double.parse(
                    (entry.value / focusEntries.length * 100)
                        .toStringAsFixed(1),
                  )
                : 0.0,
    ...
    final ageData = validAges.isEmpty
        ? <Map<String, dynamic>>[]
        : ageBucketCounts.entries
            .map(
              (entry) => {
                'label': entry.key,
                'count': entry.value,
                'percentage': validAges.isNotEmpty
                    ? double.parse(
                        (entry.value / validAges.length * 100).toStringAsFixed(
                          1,
                        ),
                      )
                    : 0.0,
    ...
    final highestAge = validAges.isEmpty
        ? null
        : validAges.reduce((current, next) => current > next ? current : next);
    final averageAge = validAges.isEmpty
        ? null
        : double.parse(
            (validAges.reduce((sum, age) => sum + age) / validAges.length)
                .toStringAsFixed(1),
          );
    final topGender = genderData.isEmpty ? null : genderData.first;
    final usingLatestAvailableDay = selectedDay == null &&
        focusEntries.isNotEmpty &&
        !_isSameDay(focusDay, today);

    return {
      'title': 'Daily Gender & Age Overview',
      'subtitle': selectedDay != null
          ? 'Gender and age profile for the selected day'
          : usingLatestAvailableDay
              ? 'Latest available day with demographic records'
              : 'Today\'s gender and age profile',
      'focus_date': focusDay.toIso8601String(),
      'focus_label': _formatAnalyticsDate(focusDay),
      'is_today': _isSameDay(focusDay, today),
      'using_latest_available_day': usingLatestAvailableDay,
      'entries_count': focusEntries.length,
      'gender_data': genderData,
      'age_data': ageData,
      'summary': {
        'total_entries': focusEntries.length,
        'gender_profiles': genderData.fold<int>(
          0,
          (sum, item) => sum + ((item['count'] as num?)?.toInt() ?? 0),
        ),
        'age_profiles': validAges.length,
        'top_gender': topGender?['label'] ?? 'No data',
        'top_gender_count': (topGender?['count'] as num?)?.toInt() ?? 0,
        'top_gender_percentage':
            (topGender?['percentage'] as num?)?.toDouble() ?? 0.0,
  ...
  List<QueueEntry> _getEntriesForDay(List<QueueEntry> entries, DateTime day) {
  ...
  String _normalizeGenderLabel(String? gender) {
    ...
    if (lower == 'prefer not to say') return 'Prefer not to say';
    if (lower == 'other') return 'Other';
    return trimmed;
  }

  String _getGenderColor(String genderLabel) {
  ...
  String _getAgeBucketLabel(int age) {
    if (age <= 17) return '17 & below';
  ...
  String _getAgeBucketColor(String bucketLabel) {
    switch (bucketLabel) {
      case '17 & below':
        return '#7C3AED';
      case '18-20':
  ...
  String _estimateWaitTime(int waiting, int serving) {
    ...
    final estimatedMinutes =
        (waiting / serving * 5).round(); // 5 min per person
