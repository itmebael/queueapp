...
class _AnalyticsScreenState extends State<AnalyticsScreen> {
  ...
  Future<void> _loadAllStudents() async {
    ...
    try {
      ...
      if (isMasterAdmin &&
        ...
        query = query.eq(
            SupabaseConfig.departmentColumn, _selectedDepartmentFilter!);
      }
      ...
      if (_selectedPurposeFilter != null &&
          _selectedPurposeFilter!.isNotEmpty) {
      ...
      _allQueueEntries =
          response.map((json) => QueueEntry.fromJson(json)).toList();
      ...
      if (isMasterAdmin) {
        ...
        print(
            'Department admin ($dept): Fetched department data from Supabase');
      }
        ...
        child: Builder(
          builder: (context) {
            ...
            return FutureBuilder<Map<String, dynamic>>(
              key: ValueKey(
                  'analytics_${_refreshKey}_${_selectedDepartmentFilter}_${_selectedCourseFilter}_${_selectedPurposeFilter}_${_selectedMonth?.toString()}_${_selectedDay?.toString()}'),
              future: _analyticsService.getComprehensiveAnalytics(
                department: isMasterAdmin
                    ...
                    : (dept != 'ALL'
                        ? dept
                        : null), // Department admin only sees their department
                course: _selectedCourseFilter,
                purpose: _selectedPurposeFilter,
                month: _selectedDay != null
                    ? null
                    : _selectedMonth, // Don't use month if day is selected
                day: _selectedDay,
              ),
              builder: (context, snapshot) {
                ...
                if (!ok) {
                  print(
                      'Analytics data not successful. Data: ${snapshot.data}');
                  print(
                      'Has data: ${snapshot.hasData}, Success: ${snapshot.data?['success']}');
                }
                ...
                // Debug logging
                print(
                    'Analytics Screen: ok=$ok, isMasterAdmin=$isMasterAdmin, graphs keys: ${graphs.keys.toList()}');
                if (isMasterAdmin) {
                  print(
                      'Analytics Screen: deptUsage count: ${deptUsage.length}, topCourses count: ${topCourses.length}');
                  ...
                  if (graphs.containsKey('department_usage')) {
                    print(
                        'Analytics Screen: department_usage graph exists: ${graphs['department_usage']}');
                  } else {
                    print(
                        'Analytics Screen: WARNING - department_usage NOT in graphs!');
                  }
                  if (graphs.containsKey('top_courses_usage')) {
                    print(
                        'Analytics Screen: top_courses_usage graph exists: ${graphs['top_courses_usage']}');
                  } else {
                    print(
                        'Analytics Screen: WARNING - top_courses_usage NOT in graphs!');
                  }
                }
                ...
                return Column(
                  ...
                  children: [
                    ...
                    if (isMasterAdmin) ...[
                      _buildDailyDemographicsChart(snapshot.data ?? {}),
                      const SizedBox(height: 20),
                    ],
                  ...
                  children: [
                    Text(
                      isMasterAdmin
                          ...
                          : '${widget.adminUser?.department} Analytics',
                      style: const TextStyle(
                        color: Colors.white,
                        fontSize: 24,
                        fontWeight: FontWeight.bold,
  ...
  Widget _buildRealTimeMetrics(Map<String, dynamic> data) {
    ...
    final waitingCount =
        _allQueueEntries.where((e) => e.status == 'waiting').length;
    final completedCount = _allQueueEntries
        .where((e) => e.status == 'completed' || e.status == 'done')
        .length;
  ...
  Widget _buildDailyDemographicsChart(Map<String, dynamic> data) {
    final graphs = data['graphs'] as Map<String, dynamic>? ?? {};
    final demographics =
        graphs['daily_demographics'] as Map<String, dynamic>? ?? {};
    final summary = demographics['summary'] as Map<String, dynamic>? ?? {};
    final genderData = ((demographics['gender_data'] as List?) ?? const [])
        .whereType<Map>()
        .map((item) => Map<String, dynamic>.from(item))
        .toList();
    final ageData = ((demographics['age_data'] as List?) ?? const [])
        .whereType<Map>()
        .map((item) => Map<String, dynamic>.from(item))
        .toList();

    if (genderData.isEmpty && ageData.isEmpty) {
      return _buildEmptyChart(
        'Daily Gender & Age Overview',
        'No same-day age or gender data available yet.',
      );
    }

    final focusLabel = demographics['focus_label'] as String? ?? 'Today';
    final subtitle = demographics['subtitle'] as String? ??
        'Same-day gender and age overview';
    final usingLatestAvailableDay =
        demographics['using_latest_available_day'] == true;
    final totalEntries = (summary['total_entries'] as num?)?.toInt() ?? 0;
    final topGender = summary['top_gender'] as String? ?? 'No data';
    final topGenderCount = (summary['top_gender_count'] as num?)?.toInt() ?? 0;
    final topGenderPercentage =
        (summary['top_gender_percentage'] as num?)?.toDouble() ?? 0.0;
    final highestAge = (summary['highest_age'] as num?)?.toInt();
    final averageAge = summary['average_age'];
    final ageProfiles = (summary['age_profiles'] as num?)?.toInt() ?? 0;
        ...
        children: [
            ...
            Container(
              ...
              child: Row(
                children: [
                  ...
                  Expanded(
                    child: Text(
                      'Today has no demographic records yet, so this chart shows the latest day with data.',
                      style: TextStyle(
                        fontSize: 12,
                        color: Colors.grey.shade700,
                      ),
          ...
          Wrap(
            ...
            children: [
              ...
              _buildDemographicSummaryChip(
                'Top Gender',
                topGenderCount > 0
                    ? '$topGender ($topGenderCount, ${_formatPercentage(topGenderPercentage)})'
                    : topGender,
                const Color(0xFFEC4899),
              ),
              _buildDemographicSummaryChip(
                'Highest Age',
                highestAge != null ? '$highestAge years old' : 'No age data',
                const Color(0xFF10B981),
              ),
              _buildDemographicSummaryChip(
                'Average Age',
                averageAge != null ? '$averageAge' : 'No age data',
                const Color(0xFFF59E0B),
              ),
              _buildDemographicSummaryChip(
                'Age Records',
                '$ageProfiles with ages',
          ...
          LayoutBuilder(
            builder: (context, constraints) {
              final stackPanels = constraints.maxWidth < 760;

              if (stackPanels) {
                return Column(
                  children: [
                    _buildDemographicPanel(
                      title: 'Gender Distribution',
                      emptyMessage: 'No gender data recorded for this day.',
                      items: genderData,
                    ),
                    const SizedBox(height: 16),
                    _buildDemographicPanel(
                      title: 'Age Distribution',
                      emptyMessage: 'No age data recorded for this day.',
                      items: ageData,
                    ),
                  ],
                );
  ...
  Widget _buildDemographicSummaryChip(String label, String value, Color color) {
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          if (items.isEmpty)
            ...
            _buildDemographicBarChart(items),
            const SizedBox(height: 16),
            Wrap(
              spacing: 10,
              runSpacing: 10,
                ...
                return Container(
                  ...
                  child: Row(
                    ...
                    children: [
                      ...
                      const SizedBox(width: 8),
                      Text(
                        '$label ($count, ${_formatPercentage(percentage)})',
                        style: const TextStyle(
                          fontSize: 12,
                          fontWeight: FontWeight.w600,
                        ),
  ...
  Widget _buildDemographicBarChart(List<Map<String, dynamic>> items) {
    ...
    final chartMax = maxValue <= 0 ? 4.0 : (maxValue + 1).ceilToDouble();
    final interval = maxValue <= 4 ? 1.0 : (chartMax / 4).ceilToDouble();
            ...
            touchTooltipData: BarTouchTooltipData(
              ...
              getTooltipItem: (group, groupIndex, rod, rodIndex) {
                ...
                return BarTooltipItem(
                  '$label\n$count record${count == 1 ? '' : 's'} • ${_formatPercentage(percentage)}',
                  const TextStyle(
                    color: Colors.white,
                    fontWeight: FontWeight.w700,
                    fontSize: 12,
  ...
  Color _parseChartColor(String? hexColor, Color fallback) {
    if (hexColor == null || hexColor.isEmpty) {
      return fallback;
    }

    final normalizedHex = hexColor.replaceFirst('#', '');
  ...
  Widget _buildDepartmentPerformanceChart(Map<String, dynamic> data) {
    ...
    if (chartData.isEmpty) {
      return _buildEmptyChart(
          'Department Performance', 'No department data available');
    }
              ...
              BarChartData(
                ...
                barGroups: chartData.asMap().entries.map((entry) {
                  ...
                  final color = Color(int.parse(
                    (dept['color'] as String?)?.replaceAll('#', '0xFF') ??
                        '0xFF263277',
                  ));
                  return BarChartGroupData(
                    x: index,
  ...
  Widget _buildTopCoursesChart(Map<String, dynamic> data) {
    final graphs = data['graphs'] as Map<String, dynamic>? ?? {};
    final coursesData =
        graphs['top_courses_usage'] as Map<String, dynamic>? ?? {};
  ...
  Widget _buildQueueStatusChart(Map<String, dynamic> data) {
    ...
    final orderedStatuses = statusCounts.keys.toList()
      ..sort(
          (a, b) => _getStatusSortOrder(a).compareTo(_getStatusSortOrder(b)));
